...
$usernamequery = "select * from restaurants where name = '$username' ";
$query = mysqli_query($con, $usernamequery);
$usernamecount= mysqli_num_rows($query);
if($usernamecount>0){
	header('location:Personal_Profile_Restaurant.php?Restaurant=$name');
}
?>
<!DOCTYPE html>
<html>
					...
					<div class="col-8">
						<div class="main-form">
							<div class="form-group">
								<input type="text" name="name" class="form-control" placeholder=" Restaurant Name(must be same as username)" value="<?php echo $_SESSION['username']?>" style="margin-right: 10vw;"  autocomplete="off" required  >
							</div>
							<div class="form-group">
